...
@Component
public class RazorpayGatewayClient implements MockGatewayClient {
    ...
    public RazorpayGatewayClient(GatewayConfig config) {
        this.config = config;
    }
    ...
    public GatewayResponse initiatePayment(Transaction transaction) {
        ...
        long latencyMs  = props != null ? props.getLatencyMs()  : 100;

        boolean accepted = random.nextInt(100) >= failureRate;
        log.debug("[RAZORPAY] txId={} orderId={} accepted={} latency={}ms",
            transaction.getTransactionId(), transaction.getOrderId(), accepted, latencyMs);

        return GatewayResponse.builder()
            .accepted(accepted)
            .gatewayTransactionId("rp_" + UUID.randomUUID().toString().substring(0, 8))
            .message(accepted ? "Payment initiated" : "Gateway error")
